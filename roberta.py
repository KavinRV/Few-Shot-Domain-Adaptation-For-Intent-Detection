...
tokenized_massive = load_dataset("carnival13/rbrt_uda_trn").remove_columns(["input"])
tokenized_eval = load_dataset("carnival13/rbrt_eval_sur").remove_columns(["input"])
jk = []
...
#         rank_score = self.rank_head(out.decoder_hidden_states[-1][:, 0, :])
        ...
        if domain_label != None:
            ...
            domain_logits = self.domain_classifier(self.grl(out.hidden_states[-1])).view(-1, self.n_pass, self.num_domain).sum(dim=1)
...
model_dir = f"rbrt_uda_ep13_norm"
